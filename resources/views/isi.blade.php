...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        ...
        <div class="flex items-center gap-2">
            <a href="/rename_folder/{{ $isi_folder->id }}" class="p-2 text-[#94a3b8] hover:text-[#2563eb] hover:bg-[#f1f5f9] rounded-lg transition-colors" title="Rename">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                </svg>
            </a>
            <a href="/permissions_folder/{{ $isi_folder->id }}" class="p-2 text-[#94a3b8] hover:text-[#2563eb] hover:bg-[#f1f5f9] rounded-lg transition-colors" title="Izin">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                </svg>
            </a>
            <a href="/hapus_folder/{{ $isi_folder->id }}" class="p-2 text-[#94a3b8] hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors" title="Hapus" onclick="return confirm('Hapus folder ini?')">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="hidden lg:flex flex-wrap items-center justify-between gap-4 mb-8 bg-white p-4 rounded-lg border border-[#e2e8f0]">
        <div class="flex items-center gap-4">
            ...
            <form action="/folder" method="POST" class="flex items-center gap-3">
                @csrf
                <input type="hidden" name="parent_id" value="{{ $isi_folder->id }}">
                <input type="text" name="nama" placeholder="Nama subfolder..." required
    ...
    <div class="lg:hidden mb-6">
        <form action="/folder" method="POST" class="flex items-center gap-2">
            @csrf
            <input type="hidden" name="parent_id" value="{{ $isi_folder->id }}">
            <input type="text" name="nama" placeholder="Tambah subfolder baru..." required
    ...
    <div class="mb-8">
        <h2 class="text-xs font-semibold text-[#64748b] uppercase tracking-wider mb-4">Subfolder</h2>
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <div class="group bg-white border border-[#e2e8f0] rounded-lg p-4 hover:border-[#2563eb] transition-colors">
                <div class="flex items-start justify-between mb-3">
                    <a href="{{ route('folder.show', $sub->id) }}" class="flex items-center gap-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            <p class="text-sm font-medium text-[#0f172a] truncate group-hover:text-[#2563eb] transition-colors">{{ $sub->nama_folder }}</p>
                ...
                <div class="flex items-center justify-end gap-1 pt-3 border-t border-[#f1f5f9]">
                    <a href="/rename_folder/{{ $sub->id }}" class="p-1.5 text-[#94a3b8] hover:text-[#2563eb] transition-colors" title="Rename">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                    </a>
                    <a href="/permissions_folder/{{ $sub->id }}" class="p-1.5 text-[#94a3b8] hover:text-[#2563eb] transition-colors" title="Izin">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/>
                        </svg>
                    </a>
                    <a href="/hapus_folder/{{ $sub->id }}" class="p-1.5 text-[#94a3b8] hover:text-red-500 transition-colors" title="Hapus" onclick="return confirm('Hapus folder ini?')">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                    </a>
    ...
    <div class="flex flex-col items-center justify-center py-20 bg-white border border-[#e2e8f0] rounded-lg text-center">
        ...
        <h3 class="text-[#0f172a] font-medium mb-1">Folder Kosong</h3>
        <p class="text-sm text-[#64748b]">Upload file atau buat subfolder di sini.</p>
    </div>
    @endif
